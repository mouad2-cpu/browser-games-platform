...
function ensureThumbnail(game: DraftGame): { thumbnail: string; copied: boolean; missing: boolean } {
  mkdirSync(THUMB_DIR, { recursive: true });
  const dest = path.join(THUMB_DIR, game.thumbFile);
  const src = path.join(ASSETS_DIR, game.coverFile);
  if (!existsSync(dest) && existsSync(src)) {
    copyFileSync(src, dest);
    return { thumbnail: `/game-covers/${game.thumbFile}`, copied: true, missing: false };
  }
  if (existsSync(dest)) {
    return { thumbnail: `/game-covers/${game.thumbFile}`, copied: false, missing: false };
  }
  return { thumbnail: `/game-covers/${game.thumbFile}`, copied: false, missing: true };
}

/** POST /api/admin/seed-popular-drafts — insert/update popular games as draft only. */
export async function POST() {
  try {
    const session = await getSession();
    requirePermission(session, PERMISSIONS.GAMES_EDIT);

    const categories = await prisma.category.findMany();
    const categoryMap = Object.fromEntries(categories.map((c) => [c.slug, c.id]));

    const results: { slug: string; action: string; missingThumb: boolean }[] = [];

    for (const game of games) {
      const { thumbnail, missing } = ensureThumbnail(game);
      ...
      if (!primaryCategoryId || categoryIds.length === 0) {
        results.push({ slug: game.slug, action: "skipped-missing-category", missingThumb: missing });
        continue;
      }
      ...
      if (existing) {
        ...
        results.push({ slug: game.slug, action: "updated-draft", missingThumb: missing });
      } else {
        ...
        results.push({ slug: game.slug, action: "created-draft", missingThumb: missing });
      }
    }

    return NextResponse.json({
      ok: true,
      note: "All games saved as draft (not published). releasedAt is null.",
      results,
    });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Failed";
    const status = message.includes("Forbidden") || message.includes("Unauthorized") ? 401 : 500;
