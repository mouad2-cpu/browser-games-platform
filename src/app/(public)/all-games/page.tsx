...
import { getAllGamesPaginated } from "@/lib/games";
...
const meta = LIST_PAGE_META["all-games"];
...
export default async function AllGamesPage({ searchParams }: Props) {
  ...
  const [{ games, total, pageSize }, dateRange] = await Promise.all([
    getAllGamesPaginated(page),
    getCatalogGameDateRange(),
  ]);
  const totalPages = Math.ceil(total / pageSize);
  const seoContent = getPageSeoContent("all-games", "All Games");
